# lark-channel-bridge-bowman

> **Bowman's fork** of [`lark-channel-bridge`](https://www.npmjs.com/package/lark-channel-bridge) with a patched `dist/cli.js`. CLI command is `lark-channel-bridge-bowman` to avoid clashing with the upstream package.
>
> **What's changed vs upstream 0.1.31:**
> - **Multimodal image input via stdin** — images are sent as `image` content blocks through `--input-format stream-json` instead of being written to disk and read back via the `Read` tool. Required for `ark-code-latest` on Volcengine Ark (`https://ark.cn-beijing.volces.com/api/coding`), where the `Read` path returns base64 in the stream but the model cannot actually see the image (silent hallucination).
> - **Magic-bytes image MIME sniffing** — fixes a bug where all images were forced to `.png` regardless of actual format. Detects JPEG / PNG / GIF / WEBP from file header.
> - Non-image attachments (audio, video, other files) still use the original `Read` path.
> - Source code is bundle-only (no `src/`); patch applied directly to `dist/cli.js`.
>
> **Known issue not yet fixed:** on Windows, using `/cd <unix-style-path>` causes session-context loss because the path isn't normalized before being compared to the cwd reported by the `claude` CLI. Workaround: use Windows-native absolute paths in `/cd`.
